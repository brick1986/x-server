...
本文把该方向落成具体协议，并回答三个随之而来的问题：分片粒度多粗（§3）、落盘怎么发现分片（§5.2）、代价多少（§3.3、§5.5）。
...
| SADD 先 + 逐条 await（Cluster） | 顺序有了 | **关不死**：drain 消费标记 → MGET 读到旧值 → SET 落地 → ack，该交错真实可达 |
| 三连 SADD→SET→SADD | 「S1 已被 drain 消费、SET 落地、崩溃于 S2 前」仍构造孤儿 | 缩小，不消除 |

结论：**单机下「SADD 先 + pipeline」是与 Lua 等价的正确协议；Cluster 下该不变量没有无原子的实现。** 且无 Lua 方案在 Cluster 下失败是**静默的**（值搁浅、无标记无日志、只在 Redis 重建时暴露），比现状的大声 `CROSSSLOT` 更危险——响亮的失败在上线前逼人修，静默的失败在线上等人发现。
...
| 分片发现 | 一次 drain | SCAN 全键空间 | **先验名单（b0000..bK-1），无需发现** |
...
- **散列对象必须是 `entity:id` 整串**：朴素 `id % K` 会被结构化 id（按服务器分块分配、snowflake 机器位）系统性倾斜；整串散列对顺序 / 块状 / snowflake 三种模式都均匀（§8.3 有统计单测钉死；哈希函数若被测试证伪则换函数，文法不动）。
...
**事实一（下界）：桶太少，Cluster 里分不均。** 桶是分配的最小颗粒（同桶必同节点）。节点负载失衡 ≈ √(节点数 ÷ 桶数)（数据量足够大时与数据量无关）。节点上限按写入吞吐估算：成熟期 10 万在线 × 每秒 1 次写、读放大 5–10 倍 → 总 ops 超单节点容量一个量级 → **16 节点封顶**。容忍度：±10% 以内的失衡在 2 倍容量冗余下无感（目标 ±7%），且 resharding 可再平衡。ε = 7% → K ≥ 16/0.07² ≈ 3265。

**事实二（上界）：K 越大，落盘每轮白跑越多。** 每轮无条件排空全部 K 个桶（§5.2），单机模式全压在一台 Redis 上：2048 / 4096 / 8192 → 每秒约 1000 / 2000 / 4000 条命令（单节点 10 万 ops/s 的 1% / 2% / 4%）。这是**改完当天就开始发生**的成本，而均衡收益只在 Cluster 才兑现。

**事实三（锁定）：K 定了就改不了。** 桶号写在每个 key 的名字里，改 K = 全量数据重写。按协议生命周期的节点上限定价，不按当前部署。

随机 tag 有生日碰撞：K 个 tag 落进 ≈ 16384·(1−e^(−K/16384)) 个不同 slot（K=4096 → ~3620，折损 12%；碰撞无害，只是结伴迁移）。K 超过 ~16384 后收益饱和。

| K | 单机背景命令 | ±失衡 @4 节点 | @8 节点 | @16 节点 | 备注 |
| --- | --- | --- | --- | --- | --- |
| 2048 | ~1000 条/s | 4.6% | 6.5% | 9.1% | 贴容忍线 |
| **4096** | **~2000 条/s** | **3.3%** | **4.7%** | **6.6%** | **选定** |
| 8192 | ~4000 条/s | 2.5% | 3.5% | 5.0% | 代价翻倍只买 1.6 个百分点 |

选错两边的活法：K 偏小 → 失衡在冗余内无感，真恶化有 resharding 兜底；K 偏大 → 多付背景命令，无正确性代价。K 钉进 `DataKeys.BUCKETS` 常量，javadoc 记录本节逻辑，分布单测（§8.3/§8.4）守卫。
...
`DRAIN_SCRIPT` 文本零改动，KEYS 从 `(DIRTY_SET, INFLIGHT_SET)` 改为 `(dirtySetOf(b), inflightSetOf(b))`——同 tag 同 slot，`RENAME` / `SUNIONSTORE` 在 Cluster 合法。落盘 spec §2.1–§2.4 的全部正确性论证**逐桶平移**：commit 与 drain 的原子交错、排空后新标记进新一轮、in-flight 残留下轮合回、恢复路径唯一——语义不变，作用域从全局变单桶。
...
- 10 万脏场景：drain 回包从「1 个 10 万成员的大包」变为「4096 个 ~24 成员的小包」，回包尺寸分布反而健康。
...
| `dbserver.dirty.buckets`（新增 gauge） | 非空桶数，同一次聚合顺带统计——落盘健康的第一信号，粗于成员数、零额外往返 |
...
2. **共置断言**：任取桶号 b，`slot("{b%04d}::dirty") == slot("{b%04d}:player:123:profile") == slot("{b%04d}::dirty:inflight")`（遍历 4096 桶）——这是 Cluster 正确性的代理验证（单机 IT 测不了 CROSSSLOT）；
...
1. **Redisson 批量执行 EVAL 的机制**（RBatch 是否支持 script eval）——第一验证点，**实现计划须先做这个 spike**。若不支持：降级为分批顺序 EVAL 并重估 K 与轮次预算（4096 × 单往返延迟须 ≤ 轮次间隔的一小半），或走 Redisson 底层连接的 pipeline。此项可能反过来调整 K 常量定值。
...
- **工程化 tag**（构造 CRC16 精确命中目标槽的字符串，消除 §3.3 的生日碰撞折损）——K 收益饱和时再评估；
